    ...
    jQuery(function() {
    	...
    	$('.uploder-container').each(function(index){
            ...
            uploader[index] = WebUploader.create({
                ...
                server: 'goodsDetail/uploadImg.do',//上传地址
                
                fileNumLimit: index==0?5:1,//一次最多上传文件个数
            	...
            	if (filePathArg.length > 0) {
            		$.each(filePathArg,function(tempIndex,item) {
                        ...
                        getFileObject(item,function(fileObject) {
                            ...
                            obj.id=tempfile.id;
                            obj.name=tempfile.name;
                            obj.type=tempfile.type;
                            obj.size=tempfile.size;
                            obj.ret=item;
                            obj.url=item;
                            obj.source=this;
                            obj.flog=true;
